# image2ppt-sdk — Claude Code 指南

image2ppt HTTP API 的官方 SDK：Python（`python/`）+ TypeScript（`typescript/`）。
拿 API key，提交图片 → 转成可编辑 PPTX。**只是一个薄 HTTP 客户端，没有任何服务端逻辑。**

## ⚠️ 这是一个公开仓库

代码会发到 PyPI / npm、推到 GitHub，任何人都能看。所以：

- **绝不**粘贴主站的服务端源码、prompt、管线细节、内部架构、任何密钥或 token。
- **依赖极简、自包含**。Python 只依赖 `requests` + `Pillow`，TypeScript 只依赖 `sharp`（上传前压图，Node 没有内置能力）+ Node 18.17+ 内置能力。除这几个之外别引第三方运行时库，更别从主仓库 import 任何东西。
- 这里只有面向 API 使用者的东西：客户端、错误类型、数据结构、上传前压图、示例。账号和计费相关的一概没有——那些全在私有主仓库。

## 🔍 每次 review / 提交前必做：内部信息泄露自查

**这是公开仓库的头号红线，优先级高于任何功能问题。** 每次 code review、以及
commit 之前，都要专门过一遍是否泄露了内部信息——不能只顾着看代码逻辑：

- **本地路径**：`~/dev/...`、`/Users/...` 之类机器上的目录布局。
- **真的密钥 / token**：API key、PyPI/npm token（`i2p_live_...` 这种占位符不算）。
- **私有主站的内部实现**：后端框架、数据库、管线阶段、prompt、架构细节，以及
  私有仓库名、私有文档名。
- 自查命令（把尖括号换成实际值再跑，**别把实际值写进本文件**）：
  `grep -rnI -E "~/dev|/Users/|<后端框架名>|<私有仓库名>" --exclude-dir=.git .`

发现疑点就停下来告诉用户，不要默默提交。**CLAUDE.md 本身也会被推到公开仓库，
它同样要过这道自查**——之前就是这个文件漏了路径和技术栈。

### 自查范围不止代码：凡是会推上去的字都算

上面那条 `grep` 只扫得到工作区里的文件，而**泄露最容易发生在文件之外**。已经栽过
一次：代码和文档全都干净，内部细节漏在了 commit message 和 PR 评论里，自查一路绿灯
放行。所以下面每一样都要过同一道红线，和代码同等对待：

- **commit message**——推上去就永久留在历史里。squash 合并只是让它不进 `main`，
  PR 页面上那些原始 commit 照样点得开。
- **PR 的标题、描述、评论，以及回复 reviewer 的每一条**。
- **issue 的正文和评论**。
- **写给 Codex / Claude 这类机器人 reviewer 的提示词**——它们的回复会公开贴在 PR 上，
  你写进提示词的东西等于自己念了一遍。

### 为了改代码读了私有源码之后，怎么说话

为改这个 SDK 去读私有仓库的源码是允许的，**但结论只能用契约的语言复述**：
可以说「45MB 按文件内容算，multipart 的包裹不计入」，因为那是调用方观察得到的行为；
不能说服务端内部是靠什么机制做到这一点的。

一句话判据：**这句话描述的是「用户调 API 会看到什么」，还是「服务端内部长什么样」？**
后者一概不写。具体不准出现的东西：

- 「去服务端源码核实过」「私有仓库里是这么写的」这类出处说明——它本身就暴露了
  内部存在什么。要说依据，就说「和 API 契约核对过」。
- 服务端内部的常量、阈值、余量的**具体数值**。对外公布的上限不在此列。
- 服务端的分层结构、有几道校验、哪一层负责什么。
- 转述或引用私有源码里的注释、变量名、函数名——哪怕只是复述大意。
- 线上运营数据：具体用户的用量、事故细节、以及能按什么维度查到什么。

## 兄弟仓库：私有主站

- 真正的产品 + API 服务端在一个**独立的私有仓库**里。本 SDK 不了解、也不该了解它的内部实现（架构、管线、技术栈都不写进这个公开仓库）。
- 两者唯一的接触点是 **API 契约**：`/api/v1` 各接口。契约文档发布在 <https://image2ppt.com/en/docs/api>（中文 <https://image2ppt.com/docs/api>），**本仓库不再保留副本**——`docs/` 下那两个文件只是指路页。以前是存副本的，副本会悄悄落后，而一旦落后，接入方读到的就是过时的那份。
- **什么时候改本仓库**：只有 `/api/v1` 契约变了（新增接口/字段/错误码）才跟着改客户端，并作为一次单独发版处理（改版本号、写 changelog、重推 PyPI/npm）。契约文档本身不在这里，不用同步。

## 常用命令

- Python：`cd python && uv sync && uv run pytest`（打包 `uv build`）
- TypeScript：`cd typescript && npm install && npm test`（构建 `npm run build`）

## 发布

包名 `image2ppt`（PyPI + npm）。发布步骤需 founder 凭证，流程在私有主仓库维护。
