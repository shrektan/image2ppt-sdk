 ...
 * is `MAX_UPLOAD_BYTES`, and `checkSubmission` compares against that exactly.
 *
 * Deliberately below the cap, and the asymmetry is the point: starting one more batch
 * costs nothing, while refusing a submission the server would have accepted is a bug in
 * the guard. Nothing here needs the headroom — the planner measures the exact bytes it
 * will send — so this is margin, not correction. **Do not turn it into a cap.**
