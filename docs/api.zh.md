...
| 字段 | 必填 | 说明 |
|---|---|---|
| `files` | 是 | 一个或多个文件。支持 `png` / `jpeg` / `webp` / `gif` / `pdf`，**单文件不超过 35MB**，且**单请求文件内容合计不超过 45MB**。同一个字段名 `files` 重复出现来传多个文件。 |
| `locale` | 否 | 成品语言环境，`zh-CN`（默认）或 `en`。 |
| `aspectRatio` | 否 | 幻灯片比例，`auto`（默认，随原图）/ `16:9` / `4:3`。 |

**页数怎么算**：一张图片算 1 页，一个 PDF 按它的实际页数算。一次提交的**总页数不能超过 50**。

**请求体积**：一次请求里的文件内容合计**不能超过 45MB**。超过会返回 `413 PAYLOAD_TOO_LARGE`；并且请注意，体积超得足够多时，网络层会在接口来得及回话之前就中断连接，此时客户端看到的是连接断开（上行慢表现为写超时，上行快表现为 broken pipe），而不是一个状态码。所以请在发出请求前自己先把合计体积算一遍，文件多就拆成几次提交。官方 SDK 这两件事都替你做了。
...
| 413 | `PAYLOAD_TOO_LARGE` | 一次请求的文件内容合计超过 45MB。 |
...
| 413 | `PAYLOAD_TOO_LARGE` | 一次请求的文件内容合计超过 45MB（提交）。 |
